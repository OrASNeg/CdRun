...
def main():
    ...
    pygame.display.set_caption('Cd Run')
    pygame.display.set_icon(Cd().sprite)
    # get some images
    ...
    buttons[2].price = 30
    # getting 5 cds
    for i in range(5): cds.append(Cd())
    # now looping through the cds list
    for cd in cds:
        cd.position.xy = random.randrange(0, DISPLAY.get_width() - cd.sprite.get_width()), cds.index(bean)*-200 - player.position.y
    ...
    while True:
        ...
        for cd in cds:
            DISPLAY.blit(cd.sprite, (cd.position.x, cd.position.y + camOffset))
        ...
        beanCountDisplay = font_small.render(str(xsCount).zfill(7), True, (0,0,0))
        DISPLAY.blit(cdCountDisplay, (72, 394))
        if dead:
        ...
        for cd in cds:
            if cd.position.y + camOffset + 90 > DISPLAY.get_height():
                cd.position.y -= DISPLAY.get_height()*2
                cd.position.x = random.randrange(0, DISPLAY.get_width() - cd.sprite.get_width())
            if (checkCollisions(player.position.x, player.position.y, player.currentSprite.get_width(), player.currentSprite.get_height(), cd.position.x, cd.position.y, cd.sprite.get_width(), cd.sprite.get_height())):
                dead = False
                cdCount += 1
                health = 100
                cd.position.y -= DISPLAY.get_height() - random.randrange(0, 200)
                cd.position.x = random.randrange(0, DISPLAY.get_width() - cd.sprite.get_width())
            ...
            if clicked and not dead and checkCollisions(mouseX, mouseY, 3, 3, buttonX, buttonY, button.sprite.get_width(), button.sprite.get_height()):
                if (beanCount >= button.price):
                    ...
                    if (buttons.index(button) == 2):
                        oldCdMultipler = cdMultiplier
                        cdMultiplier += 10
                        for i in range(cdMultiplier):
                            cds.append(Cd())
                            cds[-1].position.xy = random.randrange(0, DISPLAY.get_width() - cd.sprite.get_width()), player.position.y - DISPLAY.get_height() - random.randrange(0, 200)
